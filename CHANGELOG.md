...
### Fixed
- **Spec coverage and `@spec` gutter markers now work in any language, not just Java.** In non-Java projects (Kotlin, Go, Python, TypeScript, Rust, …) the Coverage tab reported 0% with all requirements greyed out, and the click-to-navigate gutter icons never appeared, because both surfaces were hard-gated to `.java` files. The coverage scan now reads source files of any language (skipping recognized binaries), and the gutter marker is registered for all languages. Works across the JetBrains IDE family — GoLand, PyCharm, RubyMine, WebStorm, and the rest. Fixes #18.
