...
- **Send OpenSpec Feedback action.** A new action (OpenSpec menu and the tool-window toolbar) collects a one-line message plus optional details and submits them to the OpenSpec maintainers via `openspec feedback`, running in the background with a success/error notification (the CLI's own error text on failure). The action appears only when an OpenSpec CLI is detected — feedback submission is a pure CLI capability with no built-in fallback.
