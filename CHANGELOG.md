...
- **Store and workset write actions in the Coordination tab.** With OpenSpec CLI 1.5.0 or later, the tab's Full tier now delegates create/manage actions to the CLI: create a store (`store setup`, with a required folder location) or register an existing one, unregister it, or remove it (a guarded, destructive action that deletes the store's local folder); and create a workset from a chosen set of member folders, open a workset (its members open in the IDE — the first in the current window, the rest as additional folders, behind a confirmation), or remove a saved workset (member folders are left untouched). A `store doctor`-driven health strip surfaces the highest-severity diagnostic with its suggested `fix` as an inline action. Actions run off the UI thread and surface the CLI's own error/fix text on failure, never raw output.
