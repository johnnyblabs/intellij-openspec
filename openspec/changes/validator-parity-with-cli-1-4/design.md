...
**Non-Goals:**
- Changing requirement-*name* matching semantics (the sync service's name lookups already match the whole header line case-insensitively; they keep that behavior — this change relaxes only the header token's casing in the parsing/recognition patterns, mirroring upstream).
...
3. **Keyword check evaluates the requirement body, not the header line.** Today the two surfaces disagree with each other: `BuiltInValidator` already scans only the body but reports the generic error, while `SpecFormatInspection`'s split-based scan includes the header remainder, so a header-only keyword silently passes there. Both now evaluate the body and distinguish: body-contains-keyword → OK; header-only → new diagnostic `spec-rfc-keyword-in-header` carrying the CLI's "move the keyword onto the requirement body line" guidance; nowhere → existing `spec-rfc-keywords`. For the inspection this is a strictness increase, but it matches the CLI's verdict, which is the point of parity.
