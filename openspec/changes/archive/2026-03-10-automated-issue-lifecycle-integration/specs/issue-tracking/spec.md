...
The plugin SHALL provide a PlaneService that creates and updates work items in a configured Plane instance via its REST API.
...
- **THEN** the system SHALL update the work item state to the configured "in progress" state

#### Scenario: Close work item on archive
- **WHEN** the Archive action completes for a change with a linked Plane work item
- **THEN** the system SHALL update the work item state to the configured "done" state
