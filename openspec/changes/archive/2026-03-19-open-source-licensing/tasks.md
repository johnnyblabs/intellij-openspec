...
- [x] 1.6 `scripts/setup-signing.sh` — change `REPO` to the private repo
...
- [x] 3.3 Genericize `scripts/setup-tokens.sh` — remove default internal hostnames, read only from `.env`
- [x] 3.4 Genericize `scripts/setup-signing.sh` — ensure no internal URLs leak (already clean)
- [x] 3.5 Clean `MANUAL-TASKS.md` — remove or genericize internal Forgejo/Plane hostnames, runner infrastructure details
- [x] 3.6 Clean `scripts/docs/data/plane-pages/dev-guide.md` — replace internal URLs with public GitHub URL
- [x] 3.7 Clean `scripts/PLANE_API_TEST_RESULTS.md` — remove internal Plane hostname references or exclude from public repo
