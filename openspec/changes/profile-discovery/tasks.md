...
- [ ] 8.4 Publish via `scripts/personal/setup-forgejo.sh` wiki workflow OR add to the wiki page set
...
- [ ] 9.2 Manually verify in IDE sandbox: status bar widget appears in OpenSpec project, hidden in non-OpenSpec project; click opens popup with correct content
- [ ] 9.3 Manually verify in IDE sandbox: Settings panel shows "Workflow profile:" label (no "Schema profile:" anywhere); combo lists `core` / `custom` + default; non-editable; ContextHelpLabel `?` icon present
- [ ] 9.4 Manually verify in IDE sandbox: switching profile triggers the post-switch update prompt; "Yes" runs `openspec update`; "Later" does not
- [ ] 9.5 Manually verify in IDE sandbox: a `core` user sees `Continue (custom)`, `Verify (custom)`, etc., disabled with the suffix; switching to custom removes the suffix
- [ ] 9.6 Manually verify the sync bug fix: in CLI-unavailable mode, `OpenSpecSyncAction` is enabled (was previously disabled before the `CORE_DEFAULTS` update)
- [ ] 9.7 Manually verify migration: a project with existing `openspec.xml` holding `profile = "spec-driven"` opens cleanly — combo shows orphan warning, prompts to revert on apply
